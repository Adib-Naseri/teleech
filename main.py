...
cmd = "git clone https://github.com/Adib-Naseri/teleech && bash /content/teleech/setup.sh"
proc = subprocess.run(cmd, shell=True)
cmd = "apt update && apt install ffmpeg aria2"
proc = subprocess.run(cmd, shell=True)
cmd = "pip3 install -r /content/teleech/requirements.txt"
proc = subprocess.run(cmd, shell=True)
...
with open('/content/teleech/credentials.json', 'w') as file:
    file.write(json.dumps(credentials))

Working = False

if os.path.exists("/content/teleech/my_bot.session"):
    os.remove("/content/teleech/my_bot.session") # Remove previous bot session
    
print("\rStarting Bot....")

!cd /content/teleech/ && python3 -m colab_leecher #type:ignore
